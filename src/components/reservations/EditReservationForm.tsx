...
export default function EditReservationForm({
  ...
  useEffect(() => {
    if (reservation) {
      ...
      setVehicleId(reservation.vehicle_id);
      setStartDate(reservation.start_date ? toLocalDatetimeString(new Date(reservation.start_date)) : '');
      setEndDate(reservation.end_date ? toLocalDatetimeString(new Date(reservation.end_date)) : '');
  ...
  // 1. Reactive overlap filter for vehicles (EXCLUDING self conflict)
  const availableVehicles = useMemo(() => {
    ...
    return vehicles.filter((vehicle) => {
      ...
      const hasOverlap = reservations.some((res) => {
        // CRITICAL: Exclude this reservation from conflict checks
  ...
  }, [startDate, endDate, vehicles, reservations, reservation.id]);

  // 2. Auto-calculate price, preserving manual values if inputs remain unchanged
  useEffect(() => {
    if (!startDate || !endDate || !vehicleId || !reservation) {
      return;
    }
    ...
    // Preserve original total amount if the dates and vehicle did not change
    const origStartLocal = reservation.start_date ? toLocalDatetimeString(new Date(reservation.start_date)) : '';
    const origEndLocal = reservation.end_date ? toLocalDatetimeString(new Date(reservation.end_date)) : '';

    if (
      vehicleId === reservation.vehicle_id &&
      startDate === origStartLocal &&
      endDate === origEndLocal
    ) {
      setTotalAmount(reservation.total_amount);
      return;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        addNotification('Success', 'Reservation updated successfully.', 'success');
        router.push(`/${tenantSlug}/reservations`);
      } catch (err: any) {
        console.error('Failed to update reservation:', err);
        addNotification('Error', err.message || 'Failed to update reservation.', 'error');
      }
    });
  };
