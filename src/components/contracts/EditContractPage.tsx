...
import { ArrowLeft, AlertCircle, Loader2, PenLine, RefreshCcw, Send, Lock } from 'lucide-react';
...
export function EditContractPage({ contract }: { contract: ContractWithDetails }) {
  ...
  const isLocked = statusUpper === 'ACTIVE' || statusUpper === 'RENTED' || statusUpper === 'COMPLETED';
  ...
  return (
    <div className="max-w-[672px] mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
            {isLocked ? 'Archived Contract' : 'Editing Contract'}
          </p>
          <h1 className="text-2xl font-black tracking-tight text-slate-900 font-mono">{contract.contract_number}</h1>
        </div>
        {isLocked ? (
          <span className="ms-auto text-[10px] font-bold text-slate-700 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-full uppercase tracking-wide flex items-center gap-1">
      ...
      {isLocked && (
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-600 shadow-sm">
          <Lock size={18} className="mt-0.5 shrink-0 text-slate-500" />
          <div className="text-xs font-semibold leading-relaxed">
            <span className="font-bold text-slate-800">Legal Archive:</span> This contract is currently active or completed. It is locked for editing to preserve document integrity.
          </div>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit(onSave)} className="p-6 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input type="date" {...register('start_date')} disabled={isLocked} className={`${inputClass} uppercase`} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-500">End Date</label>
              <input type="date" {...register('end_date')} disabled={isLocked} className={`${inputClass} uppercase`} />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea {...register('notes')} rows={3} placeholder="Optional internal notes…" disabled={isLocked} className={inputClass} />
          </div>
          ...
          {isLocked ? (
            <div className="pt-4 border-t border-slate-100">
              <Link
                ...
                className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold transition-all text-sm flex items-center justify-center gap-2"
              >
                Download PDF / View Rental Terms
              </Link>
            </div>
          ) : (
