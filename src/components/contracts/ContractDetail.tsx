...
export function ContractDetail({ contract, docUrls }: Props) {
  ...
  const handleCancelContract = () => {
    ...
    startTransition(async () => {
      try {
        ...
        addNotification('Success', 'Contract cancelled successfully.', 'success');
      } catch (err: any) {
        console.error('Failed to cancel contract:', err);
        addNotification('Error', err.message || 'Failed to cancel contract.', 'error');
      }
    });
  };

  const handleDeleteContract = () => {
    if (!window.confirm('Are you sure you want to delete this contract? This action is permanent.')) {
      return;
    }
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      doc.text(`${totalAmount} ${contract.currency || 'MAD'}`, 48, 116);
      ...
      doc.text(contract.currency || 'MAD', 170, 86);
    ...
    <div className="max-w-[896px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className={cn('text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wide border', statusCfg.className)}>
            {statusCfg.label}
          </span>
          {localStatus !== 'completed' && localStatus !== 'cancelled' && (
            <button
              ...
              disabled={isPending}
              className="px-4 py-2 bg-white border border-red-200 text-red-600 rounded-xl text-sm font-semibold hover:bg-red-50 transition-all flex items-center gap-2 disabled:opacity-50"
            >
              Cancel Contract
            </button>
          )}
          <button
            ...
            disabled={isPending}
            className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 border border-red-100 disabled:opacity-50"
          >
            {isPending ? (
